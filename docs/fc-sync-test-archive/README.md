...
> ⚠️ **已被取代（2026-09-11）**：本归档的内容已复活为常态化检查点工程 **`tests/sync/`**
> （ProjectReference 型，含数据沙箱隔离 + 适配后的 API + 修复的端口冲突缺陷）。
> **请勿再从本目录复跑** —— 下方「如何复跑测试」一节已失效，仅作历史留档。
> 现行运行方式：双击 `tests/sync/run-sync-tests.bat`；规则见根目录 `REGRESSION.md` 第二节。
