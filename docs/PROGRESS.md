...
> 依据：`docs/QUEST-6.md`（执行手册，只读）| `docs/TASK-v35.md`（总规划，只读）。2026-08-26 启动。
> **阶段纪律（用户确认）**：每 Phase 开发完成 → 停下 → 用户按该 Phase 验收清单验收（现象级操作）→ 通过后 git commit（message 带 "quest 6 phase N"）→ 进入下一 Phase；全部通过且用户确认后合 main + 推送双远程。全程在 `feature/todo-reminder` 开发分支。

### 回归基线（开工前提）

- [x] `dotnet build -c Debug` → Build succeeded，0 error 0 warning（2026-08-26）
- [ ] 启动应用：Alt+Space 唤出输入框保存 → 速览显示 → 设置正常（随 Phase 1 启动时一起验）

### 任务计划与验收节点

| Phase | 内容 | 验收节点（现象级） |
|---|---|---|
| 1 底座 | NoteEntry 三态 + TimeParser 规则解析 + SaveNote 类型参数 + AppSettings 6 项 + InputWindow 类型按钮/Ctrl+T + SettingsWindow 待办分组 + HotkeyService 1005 | 输入"明天上午9点 交报告"选待办 → md 出现 `(提醒: ...)`；Ctrl+T 切换；12 个时间用例；默认类型设置生效 |
| 2 面板 | UpdateTodo 原地改行 + 徽标/已办沉底 + 编辑识别建议（LLM 兜底）+ 右键设/取消提醒 + 4 档筛选 | 点徽标 → 灰显删除线 + md 含 `(状态: 已办)` + 重启保留；编辑原地改行无【编辑】行；筛选刷新后保持 |
| 3 提醒 | ReminderService + 单条弹窗 + 每日汇总 + 分组汇总窗 + 悬浮球角标 | 到点弹窗、稍后重弹、已知悉角标变红、每日汇总、自动收起、启动角标初始化 |
| 4 兼容 | 导出 `- [ ]`/`- [x]` + 同步行级验证 + 旧数据回归 | 导出格式正确；双机模拟待办行一致；历史 md 无异常；回归基线全绿 |

### Phase 状态

| Phase | 状态 | 验收结果 | commit |
|---|---|---|---|
...
**Phase 1（2026-08-27）**
- 命令类验收（执行者自跑）：`dotnet build -c Debug` 0 error 0 warning；TimeParser 12 用例全部符合期望（明天9点/今天下午3点/30分钟后/周五9点/绝对日期/8月31日晚上8点/无时间/上午9点已过/2天后/下午3点半/3点24h已过/多时间取第一个）；存储行格式 `【待办】正文 (提醒: …, 状态: …)` 组合括号正确；ParseNotes 读回 Open/Done/Read/提醒/普通行全部正确
- 用户现象级验收：**通过**（2026-08-27 用户确认）
- commit：`quest 6 phase 1: 待办数据模型+时间规则+输入框类型选择+设置项`
