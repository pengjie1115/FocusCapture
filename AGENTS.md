...
| **检查节奏(何时跑哪层)** | 中途迭代默认只跑快层(秒级);慢层仅当改动命中 `REGRESSION.md` §二触发表(或正在迭代慢层覆盖板块)才中途加跑;**全量(编译+快+慢+文档)只留给 ready,一次交付只跑一遍**。机制不选层、不跳过——省时间靠少跑重复遍数,不靠砍闸门(2026-09-21 拍板) |
...
| `daily/YYYY-MM-DD.md` | 操作流水 | append-only，带 `[Agent名]` 标识，不改他人条目；超 30 天由每周日自动化蒸馏归档 |
