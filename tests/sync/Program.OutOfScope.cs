// ═══════════════════════════════════════════════════════════════════
// 越界组 —— 原快层（tests/Program.cs）「真做事」的 6 组，2026-09-25 物理分层迁入
//
// 为什么搬：快层名义上叫「快层」，实测 49,336 ms —— 95% 的耗时压在这 6 组上，
// 而文档一直写着「纯逻辑，秒级」，腐化了整整没人发现。按 Google Small 测试的定义
// （单进程、无 I/O、无 sleep、无网络），这 6 组都不属于快桶：
//   [5]  剪贴板容错       真写系统剪贴板（本机常被其他程序占用 → 要退避重试）
//   [6]  Skill 目录扫描   真建临时目录与 SKILL.md 文件
//   [7]  运行时候选目录   真建 / 删临时目录来构造 fixture
//   [8]  子进程流式读     真起子进程（含两次等超时）
//   [9]  内置技能落地     真复制 / 备份 / 递归删除文件树
//   [10] 运行时下载器     自建本地 HTTP 服务 + 解压 + 目录清理
//
// 搬移方式：**字节级原样搬移，一条断言都没改、没删**（搬迁不是删除）。
// 与此同时，快层 csproj 移除了这些源文件的 <Compile Include> 链接 —— 于是
// 「快层源码里根本不出现这些 API」成为**机器可读的契约**，不用靠人读注释判断，
// 谁想再加回来必须同时改 csproj，而改 csproj 是显眼的动作。
//
// 本文件保留原快层编号 [5]~[10]，便于与 REGRESSION.md 触发表对照。
// ═══════════════════════════════════════════════════════════════════
...
{
    /// <summary>
    /// 截断长文本供断言说明用（与快层 tests/Program.cs 的同名局部函数同一实现，
    /// 搬迁时原样带过来 —— 慢层此前没有这个辅助函数）。
    /// </summary>
    private static string Cut(string? s) => s is null ? "(null)" : (s.Length <= 40 ? s : s[..40] + "…");

    // ══════════════════ [5] 剪贴板写入容错 SafeClipboard ══════════════════
    private static void TestClipboardFaultTolerance()
    {
    // ── [5] 剪贴板写入容错 SafeClipboard ──
    ...
    private static void TestSkillCatalogScan()
    {
    // ── [6] Skill 目录扫描与解析 SkillCatalog ──
    ...
    // ══════════════════ [7] 运行时部件候选目录 SkillRuntimeLocations ══════════════════
    private static void TestSkillRuntimeLocations()
    {
    Console.WriteLine();
    ...
    private static async Task TestSkillProcessStreaming()
    {
    // ── [8] 子进程流式读与超时保留 SkillProcess ──
    ...
    private static void TestBuiltinSkillsLifecycle()
    {
    // ── [9] 内置 Skill 落地与恢复 BuiltinSkills ──
    ...
    private static async Task TestRuntimeDownloadFlow()
    {
    // ── [10] 运行时按需下载 RuntimeDownloader ──
    ...
    try
    {
        ...
        if (File.Exists(selfDllPath) && File.Exists(selfRuntimeConfig))
        {
            ...
            // 迁入本层时补（2026-09-25）：上面那 4 个文件只够**快层**的 exe 用。
            // 本层的 exe 是 net8.0-windows + ProjectReference 主项目 —— JIT 编译 Main 时会解析
            // Main 里引用到的类型（Main 里有 FocusCapturePaths），于是启动即需要 FocusCapture.dll，
            // 它再级联带上 NAudio / PdfPig / sherpa 等。按快层原样只放 4 个文件，实测直接红
            //（说明栏：跑不起来：Unhandled exception…）。
            // 这里改为把输出目录顶层的托管程序集全部打进包 —— 那正是这个 exe 真实需要的运行时文件集。
            // 原生库住在 runtimes\ 下，--child-echo 这条路径用不到，不打。
            var bundled = new HashSet<string>(bundle.Select(b => b.Name), StringComparer.OrdinalIgnoreCase);
            foreach (var asm in Directory.GetFiles(AppContext.BaseDirectory, "*.dll"))
            {
                var n = Path.GetFileName(asm);
                if (bundled.Add(n)) bundle.Add((n, File.ReadAllBytes(asm)));
            }
...
/// <summary>
/// 同步进度收集器（随「运行时按需下载」组从快层迁入，2026-09-25）。
