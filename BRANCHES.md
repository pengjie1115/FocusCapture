...
| 分支 | 功能板块 | 状态 | 最后提交 |
|------|---------|------|---------|
| main | 主线（含 2026-09-11 自动化检查点体系） | 活跃 | f7aea64 |
| feature/sync-line-identity | 行身份改造：待办原地改行补墓碑 / 未到期待办定位全库扫行 / 行 ID 去路径 / 空上传时刻存量补齐（2026-09-12） | **开发中，未合并未推送** | 见 git log |

> 当前除 `feature/sync-line-identity` 外无其他未合并分支。**只登记「未合并的活跃分支」，合并即删即除名。**
> 该分支的迁移步骤见 [`MIGRATION.md`](MIGRATION.md)（双端升级 → 清理重复行 → 重置同步）。
