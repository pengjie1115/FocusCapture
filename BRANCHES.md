...
| feature/baidu-netdisk-cache | 百度网盘本地缓存（2026-09-16）：网盘客户端封装（设备码授权 + 上传/取回/列举/删除）、文件仓库（元数据 + 缓存账本 + 上传队列 + 淘汰器 + 附件到期清理 + 文件句柄）、Agent 文件四工具（存/取/搜/读）、设置面板「文件与网盘」板块、对话「选择文件」入口与云文件卡片、数据根自定义与迁移。**待验收，未合并** | 待验收 | b115ca6 |

> **只登记「未合并的活跃分支」，合并即删即除名。**
>
> 2026-09-16：新建 `feature/baidu-netdisk-cache`（未合并、未推送）。慢层检查点 86→107（新增「文件仓库」组 21 条）。
> 三条红线写在 REGRESSION B-14，动这块之前必读。功能与验收清单见 B-14（36 条人工验收项）。
>
