...
| feature/ai-chat-sync | AI 会话记录坚果云同步 + 历史会话管理（两阶段，方案文档《AI问答板块迭代-需求与方案文档.md》v1.1）。阶段一：数据层（SessionFile 5 字段/GUID/ChatGroupStore）+ WebDAVProvider 通用文件方法（IFileStorageProvider）+ ChatSyncEngine（整文件 LWW+Rev 冲突/E2EE 自持 DEK/删除清单闭环①）+ SyncEngine 搭车 hook/合并窗口配置化 + 退出 flush/SessionEnding/启动首拉 + 云同步页 2 设置项。阶段二（已完成）：HistoryDrawer 大改（三个点菜单/多选模式/分组分区/置顶📌/重命名）+ ChatExportService 导出四格式（复用 AgentRunService:153 过滤语义）+ 批量操作（删除/移动/导出，同名 _1/_2 不覆盖）+ ChatGroupsWindow 分组管理 + ChatTrashWindow 会话回收站（恢复/彻底删除/清空，本地行为）+ 抽屉三列布局拖宽（Thumb 改 HistoryPanel.Width 铁律）+ AiDrawerWidth 宽度记忆 + 收起按钮（Path 图标）+ Rev 冲突弹窗 + 同步后抽屉自动刷新 + 多端同名分组合并/GroupId 重映射（REGRESSION.md B-10）。+ 搭车 hook 自我死锁修复（2026-09-09，55763be） | **已合入 main（2026-09-09，随 feature/sync-cursor-fix 一并合入）** | 见 git log |
| feature/sync-cursor-fix | 笔记增量拉取游标修复（2026-09-09，从 feature/ai-chat-sync 切出含死锁修复）：根因 = push 时 UpdatedAt 用笔记原始时间戳、pull 增量按 UpdatedAt >= 游标过滤 → A 端事后同步的旧笔记时间戳小于他端游标被永久过滤（「A 传 5 条 B 只到 4 条/一条不到」存量 Bug）。修复 = SyncNote 新增 UploadedAt（上传时刻）作增量游标键（存量行空值始终投递幂等补齐）+ 三处墓碑生成补 UploadedAt + 推送时云端已有行保留原 UploadedAt 防桶漂移 + 软删重放走 RecycleBinService.AddIfAbsent 幂等落站 + 盐分叉自愈三件套（SaltNeedsUpload 持久化/盐空补传/重置写盐）+ 会话清单引号 bug（上传裸 Base64+下载兼容）+ 解密失败计数与游标保护 + 会话增量对账（LastModified stamp 跳过未变文件，治坚果云 503 限流）+ 请求节流与 503 退避重试 + 会话部分落地（中断不再全有全无）+ 退出 flush 只等笔记（includeChat=false，治 4 秒必弹窗）+ 拉取落地 CloudDataLanded 事件刷灵感速览 + 设置页「从云端重新拉取」按钮 + 密钥对齐自动清游标（REGRESSION.md B-3 累计 +11 条） | **已合入 main（2026-09-09，用户真机验收通过）** | 见 git log |
