...
| feature/baidu-netdisk-cache | 百度网盘本地缓存（2026-09-16）：网盘客户端封装（设备码授权 + 上传/取回/列举/删除）、文件仓库（元数据 + 缓存账本 + 上传队列 + 淘汰器 + 附件到期清理 + 文件句柄）、Agent 文件四工具（存/取/搜/读）、设置面板「文件与网盘」板块、对话「选择文件」入口与云文件卡片、数据根自定义与迁移。同日两轮实机事故修复（沙箱守卫粒度错位 / precreate 响应解析崩溃）已含在内。**待验收，未合并** | 待验收 | aae2ba3 |

> **只登记「未合并的活跃分支」，合并即删即除名。**
>
> 2026-09-16：新建 `feature/baidu-netdisk-cache`（未合并、未推送）。慢层检查点 86→107→121→128（「文件仓库」组 21 条 + 两轮事故回归 14 + 7 条）。
> 三条红线写在 REGRESSION B-14，动这块之前必读。功能与验收清单见 B-14（含两次事故的终态验收）。
> 同日两次上传事故：① 沙箱守卫粒度错位（「列 /apps」被自己的守卫拒掉）② precreate 响应 `block_list` 被当字符串读。
> 两段根因与教训都写在 B-14 的事故记录里 —— 这块的坑基本不在「逻辑写错」，全在「把外部约定想当然」。
>
