...
| feature/baidu-netdisk-cache | 百度网盘本地缓存（2026-09-16）：网盘客户端封装（设备码授权 + 上传/取回/列举/删除）、文件仓库（元数据 + 缓存账本 + 上传队列 + 淘汰器 + 附件到期清理 + 文件句柄）、Agent 文件四工具（存/取/搜/读）、设置面板「文件与网盘」板块、对话「选择文件」入口与云文件卡片、数据根自定义与迁移。**待验收，未合并** | 待验收 | 待提交 |
